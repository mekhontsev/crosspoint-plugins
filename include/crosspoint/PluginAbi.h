...
extern "C" uint8_t crosspoint_plugin_state_register_v5(const char* id, crosspoint_plugin::StateProvider callback, void* context);
extern "C" void crosspoint_plugin_state_unregister_v5(const char* id, void* context);
extern "C" size_t crosspoint_plugin_state_call_v5(const char* id, const uint8_t* request, size_t length, uint8_t* response, size_t capacity);
// Bounded read-only SD access: absolute path, no dot segments. Returns 1 even at EOF.
extern "C" uint8_t crosspoint_plugin_file_read_v5(const char* path, uint32_t offset, uint8_t* data, size_t capacity, size_t* length, uint32_t* total);
